...
|---|---|---|---|---|
| EarthScope FDSN (`service.iris.edu`) | NK network waveforms + station metadata | **PUBLIC, verified working** | Open FDSN terms | ObsPy client name `"EARTHSCOPE"` — `"IRIS"` is deprecated. **`SY.*` networks are synthetics and are excluded from every query** (enforced in `ghadi.fdsn`). |
...
| Real-time SeedLink latency | **B4** — unmeasured; *the* go/no-go | `scripts/seedlink_latency.py`, run for 7 days |
