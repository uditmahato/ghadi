...
|---|---|---|---|---|
| EarthScope SeedLink (`rtserve.iris.washington.edu:18000`) | NK.KKN real-time stream | **PUBLIC, verified live 2026-09-02** | Open | p95 22.8 s over 3.4 h, no interruption > 120 s — see [LATENCY.md](LATENCY.md) |
...
| Real-time SeedLink latency | **B4** — 3.4 h measured: p95 22.8 s, zero interruptions (see [LATENCY.md](LATENCY.md)); the 7-day distribution issue 0.1 asks for is still outstanding | `scripts/seedlink_latency.py`, run for 7 days |
